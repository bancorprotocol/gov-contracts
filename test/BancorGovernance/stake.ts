...
contract("BancorGovernance", async (accounts) => {
  ...
  describe("#stake()", async () => {
    it("should be able to stake 2", async () => {
      ...
      assert.strictEqual(votesAfter, (amount * decimals).toString())
    })
    ...
    it("should be able to stake 2 times 2 and have 4", async () => {
      ...
      assert.strictEqual(votesAfterSecond, (amount * 2 * decimals).toString())
    })

    it("should not be able to stake 0", async () => {
      await truffleAssert.fails(
        // stake
        governance.stake((0).toString(), {from: voter}),
